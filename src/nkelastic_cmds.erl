...
-type error() :: {elastic_error, binary(), binary()} | term().
...
request(Method, Path, Body, Opts) ->
    case request_data(Method, Path, Body, Opts) of
        {ok, _, Meta} when Method==put; Method==delete ->
            {ok, Meta};
        {ok, Data, Meta} ->
            {ok, Data, Meta};
        {error, Error} ->
...
request_data(Method, Path, Body, Opts) ->
    #{srv_id:=SrvId} = Opts,
    ClusterId = maps:get(package_id, Opts, <<"main">>),
    Path2 = iolist_to_binary([<<"/">>, Path]),
    case nkelastic:request(SrvId, ClusterId, Method, Path2, Body) of
        {ok, Resp, Time} ->
            {ok, Resp, #{time=>Time}};
        {error, Error} ->
            {error, Error}
